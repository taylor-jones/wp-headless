...
class RoboFile extends \Robo\Tasks {
	...
	public function wordpressSetup(
      ...
      'wp-description' => 'Just another (headless) WordPress site',
		...
		$confirm = $this->io()->confirm( 'This will replace your current WordPress install. Are you sure you want to do this?', false );
		...
		if ( 'y' === $config_db_answer ) {
			...
			if ( $is_darwin ) {
				...
				$this->_exec( 'sudo service mysql@5.7 start' );
			}
		} else {
			if ( $is_darwin ) {
				$this->_exec( 'brew install mysql@5.7' );
				$this->_exec( 'mysql.server start' );
    ...
    // Sync ACF
		...
		$this->wp(
			'post update 1 wp-content/themes/synergy-headless/post-content/sample-post.txt ' .
			'--post_title="Sample Post" --post_name=sample-post'
		);

    // Create homepage content
		$this->wp(
			'post create wp-content/themes/synergy-headless/post-content/welcome.txt ' .
